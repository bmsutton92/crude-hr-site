              ...
              <Reveal key={t.name} delay={i * 100} className="h-full">
                <div
                  className={`flex h-full flex-col border p-7 ${
                    t.featured
                      ? 'border-accent bg-steel/60'
                      : 'border-edge bg-steel/40'
                  }`}
                >
            ...
            {PRICING_FAQ.map((f, i) => (
              <Reveal key={f.q} delay={(i % 3) * 80}>
                <details className="group border border-edge bg-steel/40 p-6">
                  <summary className="cursor-pointer list-none font-display text-lg font-semibold uppercase tracking-wide text-body marker:content-none">
                    {f.q}
                  </summary>
